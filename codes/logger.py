...
class InfoLogger(object):
    ...
    def __init__(self, mode):
        ...
        self.__mode = mode # string, 'train' or 'valid' 两种日志模式
...
class DAELogger(InfoLogger):
    ...
    def print_log(self, epoch=None):#比较传统的日志打印函数
        if self.mode == 'train':
...
class ClassifierLogger(InfoLogger):
    ...
    def add_losses(self, loss_xw, loss_w, preds_xw, preds_w, labels):#这个函数主要是计算那四个东西
        self.add_step()
    ...
    def get_cur_losses(self):#返回计算结果
...
class MixAUSLogger(InfoLogger):
    def __init__(self, mode):
        ...
        self.__rec_loss = 0.0 # reconstruction loss重建损失
        self.__entro_loss = 0.0 # entropy loss
        self.__cl_loss_w = 0.0 # classifier loss
        self.__cl_loss_xw = 0.0
        self.__dis_loss = [] # discriminator loss 鉴别器损耗
        self.__adv_loss = [] # adversarial loss of prior generator 先前发电机的对抗性损失
        self.__latent_distance = [] # distance of prior and posteriori sampled points 先验和后验采样点的距离
        self.__factor_distance = [] # distance of prior conditioned on different mixtures 不同混合物条件下的先验距离
    ...
    def get_cur_losses(self): #计算损失并返回，为什么这么算没有细看
    ...
    def draw_curves(self): #画三个曲线
        ...
        pylab.subplot(131)
        # ------------------
        # mahalanobis distance   马氏距离，看成改进版的欧式距离即可
        ...
        # ------------------
        # mahalanobis distance of factor-priors  这个是加了影响因子的，与上面对比
        pylab.subplot(132)
        ...
        # --------------------------
        # discriminator and generator loss  鉴别器与生成机loss
        pylab.subplot(133)
