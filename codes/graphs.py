...
def get_non_pad_mask(seq, pad_idx):  #去除pad
    # seq: [B, L]
    assert seq.dim() == 2
    # [B, L]
...
def get_seq_length(seq, pad_idx):  #序列长度
...
#整个文件只有这一个类，底下都是它的成员函数
class MixPoetAUS(nn.Module):
    def __init__(self, hps): #这里初始化了参数和模型结构
        ...
        # build postional inputs to distinguish lines at different positions建立位置输入，以区分不同位置的线条
        ...
        # RNN to combine characters to form the representation of a word   RNN组合字符以形成单词的表示形式
        ...
        #--------------
        # project the decoder hidden state to a vocanbulary-size output logit  将解码器隐藏状态投影到词汇库大小输出logit
        ...
        #   classical quatrains. 这里我们使用一个二维独热向量作为嵌入o_i的输入长度，因为中国古典四行诗只有两种长度，5个字符和7个字符。
    ...
    #---------------------------------
    def set_tau(self, tau):  #set和get函数
    ...
    #---------------------------------
    def dec_step(self, inp, state, context):  #不太理解名字dec代表什么
        ...
        out = self.layers['out_proj'](cell_out)  #似乎是将输入通过模型的若干层，返回结果
    ...
    def generator(self, dec_init_state, dec_inps, lengths, specified_teach=None):
        ...
        for step in range(0, self.sens_num):
            ...
            # generate each line  生成每一行
            inp = self.bos_tensor.expand(batch_size, 1)
            for t in range(0, max_dec_len):   #每一行的字数？？
            ...
            # save each generated line  保存生成好几个可能的行
    ...
    def computer_enc(self, inps, encoder): #应该是将inps通过embed层之后，通过encoder层计算
    ...
    def forward(self, key_inps, vae_inps, dec_inps, factor_labels, factor_mask, lengths,
        use_prior=False, specified_teach=None):#前向传播
        # key_inps: (B, ken_len)
        # vae_inps: (B, poem_len)
    ...
    def rec_parameters(self):
        ...
        return chain.from_iterable(required_params) #迭代器遍历参数列表
    ...
    # ------------------------------------------------
    # functions for generating  生成时的函数，没仔细看，只知道是在计算一些东西供其他函数调用
