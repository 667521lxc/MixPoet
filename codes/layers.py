...
#这个文件应该是各种层的定义，具体使用要到graphs.py文件里。具体而言，这些层是总体模型的一个个小部件，在layers.py中被定义，然后可以graph.py文件组装使用

class BidirEncoder(nn.Module): #encoder和下面的decoder，应该是双向GRU/LSTM
    ...
    def forward(self, embed_seq, input_lens=None):
        ...
        if input_lens is None:
            ...
            packed = torch.nn.utils.rnn.pack_padded_sequence(embed_inps,
                input_lens.cpu(), batch_first=True, enforce_sorted=False)#RuntimeError: 'lengths' argument should be a 1D CPU int64 tensor, but got 1D cuda:0 Long tensor,改了好久，傻了
...
class MLP(nn.Module): #MLP，自然是若干线性层
...
class ContextLayer(nn.Module): #一个卷积层，一个线性层
...
class Discriminator(nn.Module): #鉴别器，只有几个线性层
...
class CBNLayer(nn.Module):#通过一个线性层，一个cbn层，一个激活函数
    """Contitional BatchNorm Transform Layer连续范数变换层"""
    def __init__(self, inp_size, out_size, n_classes):
        ...
        self.cbn = CondtionalBatchNorm(self.n_classes, self.out_size) #封装在这个类上层
...
class PriorGenerator(nn.Module): #先前生成器，这个生成器和后面那个不太一样，这个有两个MLP以及对应的batch normilization
...
class PosterioriGenerator(nn.Module): #后验生成器，这个只有一个MLP
    def __init__(self, inp_size, latent_size, n_class1, n_class2, factor_emb_size):
...
class CondtionalBatchNorm(nn.Module):#Conditional Batch Normalization
    _version = 2
    ...
    def forward(self, inps, labels):
        ...
        out = weight * x + bias#这里能看出这个类计算的本质。。。
...
class GumbelSampler(object):
    ...
    utilize gumbel softmax to return long type sampled labels
        instead of one-hot labels or soft  利用gumbel softmax返回长类型采样标签，而不是一个独热标签或软标签
    '''
    def __init__(self):
...
#-------------------------------------
#-------------------------------------
class LossWrapper(object): #这应该是一个专门计算loss的类
    def __init__(self, pad_idx, sens_num, sen_len):
        self.__sens_num = sens_num
        self.__sen_len = sen_len
        self.___pad_idx = pad_idx

        self.__gen_criterion = nn.CrossEntropyLoss(ignore_index=pad_idx) #生成器
        self.__cl_criterion = torch.nn.CrossEntropyLoss(reduction='none') #分类器
    ...
    def seq_ce_loss(self, outs, trgs):
        ...
        return self.__gen_criterion(output, target) #交叉熵损失


    def cross_entropy_loss(self, all_outs, all_trgs):#把all outs和all trgs汇总计算
        ...
        rec_loss = torch.cat(all_loss, dim=0)  #所有连接起来
        rec_loss = torch.mean(rec_loss) # (sens_num)  #求平均
    ...
    def cl_loss(self, logits_w, logits_xw, combined_label, mask):
        ...
        （1） 通过标记诗词，q（y | x，w）和p（y | w）都得到了优化
        （2） 对于未标记的诗歌，q（y | x，w）通过熵损失H（q（y | x，w））进行优化，p（y | w）使用从q（y | x，w）中采样的假标签进行优化
        总之, p（y | w）使用真标签和假标签进行优化，q（y | x，w）使用真实标签和entrop损失进行优化
        '''
...
class ScheduledOptim(object):
    "Optim wrapper that implements rate.   实现速率的Optim包装器    应该和优化器有关"
...
class RateDecay(object):
    '''Basic class for different types of rate decay,
        ...
        KL annealing.  不同类型速率衰减的基础类，例如，强制比、甘贝尔温度、KL退火。
    '''
...
class ExponentialDecay(RateDecay): #学习率指数衰减
...
class InverseLinearDecay(RateDecay): #反向线性衰减，，，跟学习率有关？？
...
class LinearDecay(RateDecay):#线性衰退，跟学习率有关？？
