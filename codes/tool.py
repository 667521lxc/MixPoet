...
def readPickle(data_path):#pickle加载文件
...
class Tool(object):
    ...
    # -----------------------------------
    # map functions 下面几个函数类似，都是形式转换函数，比较容易理解
    ...
    def greedy_search(self, probs):
        # probs: (B, L, V)
        outidx = [int(np.argmax(prob, axis=-1)) for prob in probs]   #实现了greedy search，每次最大那一个
        #print (outidx)
    ...
    # ----------------------------
    def get_vocab(self):  #类似于以前java里面get，set函数，规范代码
    ...
    def build_data(self, train_data_path, valid_data_path, batch_size, mode):
        '''
        Build data as batches.组装batch
        NOTE: Please run load_dic() at first.
        mode:
            cl1 or cl2: pre-train the classifier for factor 1 or factor 2    两个因素的分类器
            dae: pre-train the encoder and decoder as a denoising AutoEncoder  去噪自动编码器
        ...
        train_data = readPickle(train_data_path)   #加载数据
        valid_data = readPickle(valid_data_path)
        print (len(train_data))
        print (len(valid_data))

        # data limit for debug 调用底下那个函数
    ...
    def __build_data_core(self, data, batch_size, mode, data_limit=None):#继续调用底下那三个函数
    ...
    def build_classifier_batches(self, ori_data, batch_size, mode):
        ...
        for instance in ori_data:
            ...
            assert label >= 0
            # [keyword, sens, label]
            data.append((instance[0], instance[1], label)) #先把所有数据组成合适的格式

        batched_data = []  #将所有数据分割成多个batch
        ...
        for bi in range(0, batch_num):
            ...
            #下面是三个分量
            # build poetry batch  开始组建batch
            poems = [instance[1] for instance in instances] # all poems
            sequences = [sum(poem, []) for poem in poems]
            batch_poems = self.__build_batch_seqs(sequences, True, True) #又调用了一个函数
    ...
    def build_dae_batches(self, data, batch_size): #跟上面几乎一样
        batched_data = []
        batch_num = int(np.ceil(len(data) / float(batch_size)))
        for bi in range(0, batch_num):
            instances = data[bi*batch_size : (bi+1)*batch_size]
    ...
    def build_mixpoet_batches(self, ori_data, batch_size, mode):
        ...
        for bi in range(0, batch_num):
            ...
            batch_labels = torch.tensor(labels, dtype=torch.long)  #只有这两个转换成了tensor
    ...
    def __build_batch_length(self, lines):
        # TODO: cover more kinds of length 返回一个batch里面的各首诗的长度
        lengths = []
        for line in lines:
            yan = len(line)
    ...
    def __build_batch_seqs(self, instances, with_B, with_E, corrupt=False):  #装换成tensor
        # pack sequences as a tensor
        seqs = self.__get_batch_seq(instances, with_B, with_E, corrupt) #调用下面的函数
        seqs_tensor =self.__sens2tensor(seqs)
        return seqs_tensor


    def __get_batch_seq(self, seqs, with_B, with_E, corrupt):#把一个batch内的数据整理到相同的长度
        batch_size = len(seqs)
        ...
        for i in range(0, batch_size):
            ...
            new_seq = [self.__B_ID] * int(with_B) + seq\
                + [self.__E_ID] * int(with_E) + pads   #把padding加在整条数据最后面

            batched_seqs.append(new_seq)  #把一条条数据加进batch里面
    ...
    def __do_corruption(self, inp):
        # corrupt the sequence by setting some tokens as UNK   通过将某些令牌设置为UNK来破坏序列（随机mask）
        ...
        pos = random.sample(list(range(0, len(inp))), m)  #没有看明白把哪一些UNK了
        for p in pos:
    ...
    def keys2tensor(self, keys): #把id转成tensor
        key_idxes = []
        for key in keys:
    ...
    def lengths2tensor(self, lengths):  #把长度转成tensor
        vec = []
    ...
    def pos2tensor(self, step):     #表示哪一行的向量转成tensor
